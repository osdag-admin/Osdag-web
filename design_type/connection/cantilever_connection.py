...
class CantileverConnection(ShearConnection):

    def __init__(self):
        super(CantileverConnection, self).__init__()
    ...
    def tab_value_changed(self):
        ...
        t6 = (KEY_DISP_COLSEC, [KEY_SUPTNGSEC], [
              KEY_SOURCE], TYPE_TEXTBOX, self.change_source)
        change_tab.append(t6)

        t7 = (KEY_DISP_BEAMSEC, [KEY_SUPTDSEC], [
              KEY_SOURCE], TYPE_TEXTBOX, self.change_source)
    ...
    def input_dictionary_without_design_pref(self):
        design_input = []
        t1 = (KEY_MATERIAL, [KEY_SUPTNGSEC_MATERIAL,
              KEY_SUPTDSEC_MATERIAL], 'Input Dock')
        design_input.append(t1)
    ...
    def set_osdaglogger(self, key):
        """
        Function to set Logger for Cantilever Module
        """
        # @author Arsil Zunzunia
        # super(CantileverConnection, CantileverConnection).set_osdaglogger(key)
        global logger
    ...
    def input_values(self):
        ...
        e.g.
        t = (Key, Key_display, Type, existing_val,
             Current_Value, enabled/disabled, Validator_type)
        '''

        # @author: Amir, Umair
        self.module = KEY_DISP_CANTILEVER

        options_list = []

        t16 = (KEY_MODULE, KEY_DISP_CANTILEVER,
        ...
        t2 = (KEY_CONN, KEY_DISP_CONN, TYPE_COMBOBOX,
              VALUES_CONN, True, 'No Validator')
        options_list.append(t2)

        t15 = (KEY_IMAGE, None, TYPE_IMAGE,
               "./ResourceFiles/images/fin_cf_bw.png", True, 'No Validator')
        options_list.append(t15)

        t3 = (KEY_SUPTNGSEC, KEY_DISP_COLSEC, TYPE_COMBOBOX,
              connectdb("Columns"), True, 'No Validator')
        options_list.append(t3)

        t4 = (KEY_SUPTDSEC, KEY_DISP_BEAMSEC, TYPE_COMBOBOX,
              connectdb("Beams"), True, 'No Validator')
        ...
        t14 = (KEY_PLATETHK, KEY_DISP_PLATETHK, TYPE_COMBOBOX_CUSTOMIZED,
               VALUES_PLATETHK, True, 'No Validator')
    ...
    def capacities(self, status):
        ...
        t00 = (None, "", TYPE_NOTE,
               "Representative image for Failure Pattern (Half Plate)- 2 x 3 Bolts pattern considered")
        ...
        t17 = (KEY_OUT_PLATE_SHEAR, KEY_OUT_DISP_PLATE_SHEAR, TYPE_TEXTBOX, round(
            self.plate.shear_yielding_capacity/1000, 2) if status else '')
        capacities.append(t17)

        t18 = (KEY_OUT_PLATE_RUPTURE, KEY_OUT_DISP_PLATE_RUPTURE, TYPE_TEXTBOX, round(
            self.plate.shear_rupture_capacity/1000, 2) if status else '')
        capacities.append(t18)

        t17 = (KEY_OUT_PLATE_BLK_SHEAR, KEY_OUT_DISP_PLATE_BLK_SHEAR, TYPE_TEXTBOX, round(
            self.plate.block_shear_capacity_shear/1000, 2) if status else '')
        capacities.append(t17)
        ...
        t19 = (KEY_OUT_PLATE_MOM_DEMAND, KEY_OUT_DISP_PLATE_MOM_DEMAND, TYPE_TEXTBOX, round(
            self.plate.moment_demand/1000000, 2) if status else '')
        capacities.append(t19)

        t20 = (KEY_OUT_PLATE_MOM_CAPACITY, KEY_OUT_DISP_PLATE_MOM_CAPACITY, TYPE_TEXTBOX, round(
            self.plate.moment_capacity/1000000, 2) if status else '')
    ...
    def section_capacities(self, status):
        ...
        t17 = (KEY_SHEAR_YIELDCAPACITY, KEY_OUT_DISP_PLATE_SHEAR, TYPE_TEXTBOX, round(
            self.supported_section.shear_yielding_capacity/1000, 2) if status else '')
        capacities.append(t17)

        t18 = (KEY_SHEAR_RUPTURECAPACITY, KEY_OUT_DISP_PLATE_RUPTURE, TYPE_TEXTBOX, round(
            self.supported_section.shear_rupture_capacity/1000, 2) if status else '')
        capacities.append(t18)

        t17 = (KEY_SHEAR_BLOCKSHEARCAPACITY, KEY_OUT_DISP_PLATE_BLK_SHEAR, TYPE_TEXTBOX, round(
            self.supported_section.block_shear_capacity_shear/1000, 2) if status else '')
        capacities.append(t17)
        ...
        t20 = (KEY_MEMBER_MOM_CAPACITY, KEY_OUT_DISP_PLATE_MOM_CAPACITY, TYPE_TEXTBOX, round(
            self.supported_section.moment_capacity / 1000000, 2) if status else '')
        capacities.append(t20)
    ...
    def output_values(self, flag):
        ...
        t3 = (KEY_OUT_GRD_PROVIDED, KEY_OUT_DISP_GRD_PROVIDED,
              TYPE_TEXTBOX, self.bolt.bolt_grade_provided if flag else '', True)
        ...
        t6 = (KEY_OUT_BOLT_CAPACITY, KEY_OUT_DISP_BOLT_CAPACITY, TYPE_TEXTBOX, round(
            self.bolt.bolt_capacity/1000, 2) if flag else '', True)
        out_list.append(t6)

        t21 = (KEY_OUT_BOLT_FORCE, KEY_OUT_DISP_BOLT_FORCE, TYPE_TEXTBOX,
               round(self.plate.bolt_force / 1000, 2) if flag else '', True)
        out_list.append(t21)

        t7 = (KEY_OUT_BOLT_LINE, KEY_OUT_DISP_BOLT_LINE, TYPE_TEXTBOX,
              self.plate.bolt_line if flag else '', True)
        out_list.append(t7)

        t8 = (KEY_OUT_BOLTS_ONE_LINE, KEY_OUT_DISP_BOLTS_ONE_LINE,
              TYPE_TEXTBOX, self.plate.bolts_one_line if flag else '', True)
        out_list.append(t8)
        ...
        t14 = (KEY_OUT_PLATETHK, KEY_OUT_DISP_PLATETHK, TYPE_TEXTBOX,
               int(self.plate.thickness_provided) if flag else '', True)
        out_list.append(t14)

        t15 = (KEY_OUT_PLATE_HEIGHT, KEY_OUT_DISP_PLATE_HEIGHT,
               TYPE_TEXTBOX, float(self.plate.height) if flag else '', True)
        out_list.append(t15)

        t16 = (KEY_OUT_PLATE_LENGTH, KEY_OUT_DISP_PLATE_LENGTH,
               TYPE_TEXTBOX, float(self.plate.length) if flag else '', True)
        out_list.append(t16)

        t22 = ('button1', KEY_OUT_DISP_PLATE_CAPACITIES, TYPE_OUT_BUTTON, [
               'Capacity Details', self.capacities], True)
        out_list.append(t22)

        t13 = (None, DISP_TITLE_SECTION, TYPE_TITLE, None, True)
        out_list.append(t13)

        t22 = ('button2', KEY_OUT_DISP_PLATE_CAPACITIES, TYPE_OUT_BUTTON, [
               'Capacity Details', self.capacities], True)
        out_list.append(t22)
        ...
        t14 = (KEY_OUT_WELD_SIZE, KEY_OUT_DISP_WELD_SIZE,
               TYPE_TEXTBOX, self.weld.size if flag else '', True)
        out_list.append(t14)

        t15 = (KEY_OUT_WELD_STRENGTH, KEY_OUT_DISP_WELD_STRENGTH,
               TYPE_TEXTBOX, round(self.weld.strength, 2) if flag else '', True)
        out_list.append(t15)

        t16 = (KEY_OUT_WELD_STRESS, KEY_OUT_DISP_WELD_STRESS,
               TYPE_TEXTBOX, round(self.weld.stress, 2) if flag else '', True)
        out_list.append(t16)
    ...
    def set_input_values(self, design_dictionary):
        ...
        super(CantileverConnection, self).set_input_values(design_dictionary)
        ...
        self.weld = Weld(
            material_g_o=design_dictionary[KEY_DP_WELD_MATERIAL_G_O], fabrication=design_dictionary[KEY_DP_WELD_FAB])
        print("input values are set. Doing preliminary member checks")
        # self.warn_text(self)
        self.member_capacity()

    def member_capacity(self):
        super(CantileverConnection, self).member_capacity()
        self.thickness_possible = []
        self.supported_section.low_shear_capacity = round(
            0.6 * self.supported_section.shear_yielding_capacity, 2)
        if self.supported_section.low_shear_capacity / 1000 > self.load.shear_force and \
            ...
            print(
                "Preliminary member check(s) have passed. Checking available bolt diameter(s).")
            self.thickness_possible = [
                i for i in self.plate.thickness if i >= self.supported_section.web_thickness]
    ...
    def select_bolt_dia(self):
        ...
        self.max_plate_height = self.supported_section.max_plate_height(
            self.connectivity, 50.0)

        self.res_force = math.sqrt(
            self.load.shear_force ** 2 + self.load.axial_force ** 2) * 1000

        self.plate.thickness_provided = min(self.thickness_possible)
        self.plate.connect_to_database_to_get_fy_fu(
            grade=self.plate.material, thickness=self.plate.thickness_provided)
        ...
        self.bolt_conn_plates_t_fu_fy.append(
            (self.plate.thickness_provided, self.plate.fu, self.plate.fy))
        self.bolt_conn_plates_t_fu_fy.append(
        ...
        for self.bolt.bolt_diameter_provided in reversed(self.bolt.bolt_diameter):
            ...
            self.bolt.calculate_bolt_capacity(bolt_diameter_provided=self.bolt.bolt_diameter_provided,
                                              ...
                                              conn_plates_t_fu_fy=self.bolt_conn_plates_t_fu_fy,
                                              n_planes=1)
            print("getting web plate details for dia:", self.bolt.bolt_diameter_provided,
                  self.bolt.bolt_grade_provided, self.bolt.bolt_capacity)
    ...
    def get_bolt_grade(self):
        ...
        for self.bolt.bolt_grade_provided in reversed(self.bolt.bolt_grade):
            ...
            print("for grade :", self.bolt.bolt_grade_provided, "capacity is:",
                  self.bolt.bolt_capacity, "force is:", self.plate.bolt_force)
    ...
    def get_fin_plate_details(self):
        ...
        for self.plate.thickness_provided in self.thickness_possible:
            ...
            if self.connectivity in VALUES_CONN_1:
                self.weld_connecting_plates = [
                    self.supporting_section.flange_thickness, self.plate.thickness_provided]
            else:
                self.weld_connecting_plates = [
                    self.supporting_section.web_thickness, self.plate.thickness_provided]
            ...
            if available_welds:
                ...
                while self.supported_section.design_status == False or self.plate.design_status_capacity == False or \
                    ...
                    if self.supported_section.moment_capacity > self.plate.moment_demand and self.plate.height+10 <= self.max_plate_height:
                        ...
                        h_recalc = (self.plate.gauge_provided + 5) * \
                        ...
                        if self.plate.block_shear_capacity_axial > self.load.axial_force*1000 or \
                            ...
                            self.plate.height = (self.plate.gauge_provided) * (
                                self.plate.bolts_one_line - 1) + self.plate.edge_dist_provided
                            self.plate.get_web_plate_details(self.bolt.bolt_diameter_provided, self.plate.height, self.plate.height,
        ...
        if self.load.shear_force*1000 > self.supported_section.shear_capacity:
            ...
            logger.error(": The shear capacity of the beam is less than the applied shear force, %2.2f kN [Ref. Cl.6.4.1, IS 800:2007]."
                         % self.load.shear_force)
            logger.warning(": The shear capacity of the beam is {} kN.".format(
        ...
        if self.load.axial_force*1000 > self.supported_section.tension_capacity:
            ...
            logger.error(": The tensile capacity of the beam is less than the applied axial force, %2.2f kN [Ref. Cl. 6.4.1, IS 800:2007]."
                         % self.load.axial_force)
            logger.warning(": The tensile capacity of the beam is {} kN." .format(
        ...
        if self.plate.moment_capacity < self.plate.moment_demand:
            ...
            logger.error("Plate moment capacity is less than the moment demand, {} kNm [cl. 8.2.1.2]"
                         .format(round(self.plate.moment_demand/1000000, 2)))
            logger.warning(":Moment capacity of plate is {} kN-m" .format(
            ...
            self.logs.append({
                "msg": "Plate moment capacity is less than the moment demand, {} kNm [cl. 8.2.1.2]"
                .format(round(self.plate.moment_demand/1000000, 2)),
                "type": "error"
            })
            self.logs.append({
        ...
        if self.weld.strength < self.weld.stress:
            ...
            self.weld.design_status = False
            logger.error(
                ": The weld thickness is not sufficient [Ref. Cl. 10.5.7, IS 800:2007].")
            logger.warning(": The weld stress is {} N/mm and the weld strength is {} N/mm.".format(
    ...
    def section_shear_checks(self):
        ...
        A_vn = ((n_row - 1) * pitch + end - (float(n_row) - 0.5)
                * bolt_hole_dia) * web_thick
        A_tg = ((n_col - 1) * gauge + edge) * web_thick
        A_tn = ((n_col - 1) * gauge + edge -
                (float(n_col) - 0.5) * bolt_hole_dia) * web_thick
        ...
        A_vn = (self.supported_section.web_height - float(n_row) *
                bolt_hole_dia) * self.supported_section.web_thickness
        self.supported_section.shear_rupture_capacity = AISC.cl_j_4_2_b_shear_rupture(
            A_vn, self.supported_section.fu)
        ...
        if self.supported_section.shear_capacity < self.load.shear_force * 1000:
            ...
            self.logs.append({
                "msg": "The shear capacity of the section is guiding plate height, current height is {} mm.".format(self.plate.height),
                "type": "warning"
            })
        else:
            self.supported_section.design_status = True

        self.supported_section.tension_rupture_capacity = IS800_2007.cl_6_3_1_tension_rupture_strength(
            A_vn, self.supported_section.fu)
        A_tg = ((n_row - 1) * pitch) * web_thick
        A_tn = ((n_row - 1) * pitch - (float(n_row) - 1.0)
                * bolt_hole_dia) * web_thick
        A_vg = 2 * ((n_col - 1) * gauge + edge) * web_thick
        A_vn = 2 * ((n_col - 1) * gauge + edge -
                    (float(n_col) - 0.5) * bolt_hole_dia) * web_thick
        ...
        if self.supported_section.tension_capacity < self.load.axial_force * 1000:
            ...
            self.logs.append({
                "msg": 'The tension capacity of the section is guiding plate height, current height is {} mm.' .format(self.plate.height),
                "type": "warning"
            })
        else:
            self.supported_section.design_status = True

        self.supported_section.moment_capacity = IS800_2007.cl_8_2_1_2_design_moment_strength(self.supported_section.elast_sec_mod_z,
                                                                                              self.supported_section.plast_sec_mod_z, self.supported_section.fy, 'plastic')
        ...
        if self.supported_section.moment_capacity < self.plate.moment_demand:
            logger.warning(
                ...
                'section.')
            self.logs.append({
                "msg": 'The moment capacity of the section is less than moment demand, choose a bigger section or increase the material strength of the ',
                "type": "warning"
            })
            self.supported_section.design_status = False
        else:
        ...
        if self.supported_section.IR > 1 or self.supported_section.moment_capacity < self.plate.moment_demand or\
            ...
            self.logs.append({
                "msg": 'Axial - Moment interaction ratio of section is guiding plate height, current height is {} mm.' .format(self.plate.height),
                "type": "warning"
            })
            self.supported_section.design_status = False
        else:
    ...
    def plate_shear_checks(self):
        ...
        plate_A_vn = ((n_row - 1) * pitch + end -
                      (float(n_row) - 0.5) * bolt_hole_dia) * p_th
        plate_A_tg = ((n_col - 1) * gauge + edge) * p_th
        plate_A_tn = ((n_col - 1) * gauge + edge -
                      (float(n_col) - 0.5) * bolt_hole_dia) * p_th
        ...
        self.plate.shear_yielding_capacity = IS800_2007.cl_8_4_design_shear_strength(
            A_vg, self.plate.fy)
        ...
        self.plate.shear_rupture_capacity = AISC.cl_j_4_2_b_shear_rupture(
            A_vn, self.plate.fu)
        ...
        self.plate.tension_yielding_capacity = IS800_2007.cl_6_2_tension_yielding_strength(
            A_g, self.plate.fy)

        A_n = (self.plate.height - self.plate.bolt_line *
               self.bolt.dia_hole) * self.plate.thickness_provided

        self.plate.tension_rupture_capacity = IS800_2007.cl_6_3_1_tension_rupture_strength(
            A_n, self.plate.fu)
        plate_A_tg = ((n_row - 1) * pitch) * p_th
        plate_A_tn = ((n_row - 1) * pitch - (float(n_row) - 1.0)
                      * bolt_hole_dia) * p_th
        plate_A_vg = 2 * ((n_col - 1) * gauge + edge) * p_th
        plate_A_vn = 2 * ((n_col - 1) * gauge + edge -
                          (float(n_col) - 0.5) * bolt_hole_dia) * p_th
        ...
        if self.plate.tension_capacity < self.load.axial_force*1000:
            ...
            self.logs.append({
                "msg": 'The tension capacity of the plate is guiding the plate height, current height is {} mm.' .format(self.plate.height),
                "type": "warning"
            })
        else:
            self.plate.design_status_capacity = True
        ...
        self.plate.moment_capacity = IS800_2007.cl_8_2_1_2_design_moment_strength(
            Z_e, Z_p, self.plate.fy, 'plastic')

        if self.plate.moment_capacity < self.plate.moment_demand:
            logger.warning(
                'The moment capacity of the plate is guiding plate height, current height is {} mm.'.format(self.plate.height))
            self.logs.append({
                "msg": 'The moment capacity of the plate is guiding plate height, current height is {} mm.'.format(self.plate.height),
                "type": "warning"
            })
            self.plate.design_status_capacity = False
        else:
        ...
        self.plate.IR = round(self.plate.moment_demand/self.plate.moment_capacity + (
            self.load.axial_force*1000)/self.plate.tension_capacity, 2)
        if self.plate.IR > 1 or self.plate.shear_capacity < self.load.shear_force*1000 or self.plate.moment_capacity < self.plate.moment_demand:
            ...
            self.logs.append({
                "msg": 'Moment-Axial interaction ratio of plate is guiding plate height, current height is {} mm.'.format(self.plate.height),
                "type": "warning"
            })
            self.plate.design_status_capacity = False
        else:
    ...
    def get_available_welds(self, connecting_members=[]):
        ...
        weld_size_min = math.ceil(IS800_2007.cl_10_5_2_3_min_weld_size(
            connecting_members[0], connecting_members[1]))
        ...
        if weld_size_max == weld_size_min:
            logger.info("The minimum weld size is greater than or equal to the thickness of the thinner connecting plate [Ref. Table 21, "
                        "IS800:2007].")
            self.logs.append({
                "msg": "The minimum weld size is greater than or equal to the thickness of the thinner connecting plate [Ref. Table 21, ",
                "type": "info"
            })
            logger.info(
                "Thicker plate shall be adequately preheated to prevent cracking of the weld.")
            self.logs.append({
                "msg": "Thicker plate shall be adequately preheated to prevent cracking of the weld.",
                "type": "info"
            })

        available_welds = list(
            [x for x in ALL_WELD_SIZES if (weld_size_min <= x <= weld_size_max)])
    ...
    def get_design_status(self):
        print("plate design status is ", self.plate.design_status,
              "weld status is", self.weld.design_status)
    ...
    def save_design(self, popup_summary):
        ...
        t1 = ('SubSection', 'Initial Section Check',
        ...
        t1 = (KEY_DISP_SHEAR_YLD, self.load.shear_force,
              cl_8_4_shear_yielding_capacity_member(
                  h, t, a.fy, gamma_m0, round(a.shear_yielding_capacity / 1000, 2)),
        ...
        t1 = (KEY_DISP_ALLOW_SHEAR, self.load.shear_force,
              allow_shear_capacity(
                  round(a.shear_yielding_capacity/1000, 2), round(a.low_shear_capacity/1000, 2)),
        ...
        t1 = (KEY_DISP_TENSION_YIELDCAPACITY, self.load.axial_force,
              cl_6_2_tension_yield_capacity_member(
                  h, t, a.fy, gamma_m0, round(a.tension_yielding_capacity / 1000, 2)),
        ...
        if not self.thickness_possible and self.supported_section.design_status_initial is True:
            t1 = ('SubSection', 'Minimum Plate Thickness Check',
                  '|p{4cm}|p{5cm}|p{5.5cm}|p{1.5cm}|')
            self.report_check.append(t1)
            ...
            min_shear_load = min(
                40, round(0.15 * self.supported_section.shear_yielding_capacity / 0.6, 2))
            applied_shear_force = max(self.load.shear_force, min_shear_load)

            t1 = (KEY_DISP_APPLIED_AXIAL_FORCE,
                  self.load.axial_force, self.load.axial_force, "")
            self.report_check.append(t1)
            ...
            connecting_plates = [self.plate.thickness_provided,
            ...
            t1 = ('SubSection', 'Bolt Design',
                  '|p{3.5cm}|p{5.3cm}|p{6.7cm}|p{1.5cm}|')
            self.report_check.append(t1)
            ...
            t6 = (DISP_NUM_OF_COLUMNS, '', self.plate.bolt_line,
                  get_pass_fail(2, self.plate.bolt_line, relation='geq'))
            ...
            if self.plate.design_status is False:
                ...
                t10 = (KEY_OUT_REQ_PARA_BOLT, parameter_req_bolt_force(bolts_one_line=self.plate.bolts_one_line, gauge=self.plate.gauge_provided,
                                                                       ymax=round(
                                                                           self.plate.ymax, 2),
                                                                       xmax=round(
                                                                           self.plate.xmax, 2),
                ...
                t10 = (KEY_OUT_DISP_BOLT_FORCE, Vres_bolts(bolts_one_line=self.plate.bolts_one_line,
                                                           ...
                                                           bolt_line=self.plate.bolt_line,
                                                           shear_load=round(
                                                               self.load.shear_force, 2),
                                                           axial_load=round(
                                                               self.load.axial_force, 2),
                                                           moment_demand=round(
                                                               self.plate.moment_demand / 1000000, 2),
                                                           r=round(
                                                               self.plate.sigma_r_sq / 1000, 2),
                                                           vbv=round(
                                                               self.plate.vbv / 1000, 2),
                                                           tmv=round(
                                                               self.plate.tmv / 1000, 2),
                                                           tmh=round(
                                                               self.plate.tmh / 1000, 2),
                                                           abh=round(
                                                               self.plate.abh / 1000, 2),
                ...
                if self.bolt.bolt_type == TYP_BEARING:
                    ...
                    t3 = (KEY_OUT_DISP_BOLT_CAPACITY, '',
                          cl_10_3_2_bolt_capacity(
                              bolt_shear_capacity_kn, bolt_bearing_capacity_kn, bolt_capacity_kn),
                          '')
                    self.report_check.append(t3)
                else:
                ...
                t1 = ('SubSection', 'Plate Design',
                      '|p{3.5cm}|p{5cm}|p{6cm}|p{1.5cm}|')
                self.report_check.append(t1)
                ...
                #######################
                self.plate.plast_sec_mod_z = self.plate.height ** 2 * \
                    self.plate.thickness_provided / 4
                for a in [self.plate, self.supported_section]:
                    ...
                    t1 = (KEY_DISP_PLATE_BLK_SHEAR_SHEAR, '', cl_6_4_blockshear_capacity_member(
                        Tdb=round(a.block_shear_capacity_shear / 1000, 2), stress='shear'), '')
                    self.report_check.append(t1)

                    t1 = (KEY_DISP_SHEAR_CAPACITY, self.load.shear_force,
                          cl_8_4_shear_capacity_member(round(a.low_shear_capacity / 1000, 2),
                                                       round(
                                                           a.shear_rupture_capacity / 1000, 2),
                    ...
                    t1 = (KEY_DISP_PLATE_BLK_SHEAR_TENSION, '', cl_6_4_blockshear_capacity_member(
                        Tdb=round(a.block_shear_capacity_axial / 1000, 2), stress='axial'), '')
                    self.report_check.append(t1)
                    ...
                    t1 = (KEY_DISP_TENSION_CAPACITY, self.load.axial_force,
                          cl_6_1_tension_capacity_member(round(a.tension_yielding_capacity / 1000, 2),
                                                         round(
                                                             a.tension_rupture_capacity / 1000, 2),
                ...
                weld_conn_plates_fu = [
                    self.supporting_section.fu, self.plate.fu]
                ...
                if Ip_weld != 0.0:
                    t1 = (DISP_WELD_STRENGTH, weld_strength_req(V=self.load.shear_force*1000, A=self.load.axial_force*1000,
                          ...
                          cl_10_5_7_1_1_weld_strength(
                              weld_conn_plates_fu, gamma_mw, self.weld.throat_tk, self.weld.strength),
