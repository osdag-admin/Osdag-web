...
def create_3d_model(design_output, save_path):
    try:
        # Create basic shapes based on design outputs
        beam = BRepPrimAPI_MakeBox(
            design_output['beam_length'],
            design_output['beam_width'],
            design_output['beam_height']
        ).Shape()
        
        # Use forward slashes and ensure directory exists
        base_path = "file_storage/cad_models"
        os.makedirs(base_path, exist_ok=True)
        
        # Create STEP file with proper path
        filename = f"model_{uuid.uuid4()}.step"
        full_path = os.path.join(base_path, filename).replace("\\", "/")
        
        # Write to STEP file
        writer = STEPControl_Writer()
        status = writer.Transfer(beam, STEPControl_AsIs)
        if status == IFSelect_RetDone:
            status = writer.Write(full_path)
            
        return os.path.join('cad_models', filename)
