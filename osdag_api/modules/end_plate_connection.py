...
# def perform_design_calculations(input_data):
#     """
#     Perform design calculations using input data from frontend
#     """
#     try:
#         # Transform frontend data to required format
#         transformed_input = {
#             "Bolt.Bolt_Hole_Type": "Standard",
#             "Bolt.Diameter": input_data["boltDiameter"],
#             "Bolt.Grade": input_data["propertyClass"],
#             "Bolt.Slip_Factor": "0.3",
#             "Bolt.TensionType": "Pretensioned",
#             "Bolt.Type": input_data["boltType"],
#             "Connectivity": input_data["connectivity"],
#             "Connector.Material": input_data["material"],
#             "Design.Design_Method": "Limit State Design",
#             "Detailing.Corrosive_Influences": "No",
#             "Detailing.Edge_type": "Sheared or hand flame cut",
#             "Detailing.Gap": "2",
#             "Load.Axial": str(input_data["Load.Axial"]),
#             "Load.Shear": str(input_data["Load.Shear"]),
#             "Load.Moment": str(input_data["Load.Moment"]),
#             "Material": input_data["material"],
#             "Member.Supported_Section.Designation": input_data["beamSection"],
#             "Member.Supported_Section.Material": input_data["material"],
#             "Member.Supporting_Section.Designation": input_data["beamSection"],
#             "Member.Supporting_Section.Material": input_data["material"],
#             "Module": "Beam-Beam End Plate",
#             "Weld.Fab": "Shop Weld",
#             "Weld.Material_Grade_OverWrite": "410",
#             "Connector.Plate.Thickness_List": input_data["endPlateThickness"]
#         }

#         # Validate input data
#         validate_input(transformed_input)

#         # Create module and generate output
#         output_values, logs = generate_output(transformed_input)
...
def perform_design_calculations(input_data):
    """Perform design calculations with improved input validation"""
    try:
        # Better bolt diameter handling with validation
        def validate_bolt_diameter(value):
            if isinstance(value, (list, tuple)):
                return [str(int(float(x))) for x in value if str(x).replace('.', '').isdigit()]
            elif isinstance(value, (int, float, str)):
                return [str(int(float(value)))]
            return []

        # Transform frontend data with validated bolt diameter
        bolt_diameter = validate_bolt_diameter(input_data.get("boltDiameter", []))
        
        # Validate bolt diameter list is not empty
        if not bolt_diameter:
            raise ValueError("Bolt diameter must be a non-empty list of numeric values")

        transformed_input = {
            "Bolt.Bolt_Hole_Type": "Standard",
            "Bolt.Diameter": bolt_diameter,  # Now properly formatted as List[str]
            "Bolt.Grade": [str(g) for g in input_data.get("propertyClass", [])],
            "Bolt.Slip_Factor": "0.3",
            "Bolt.TensionType": "Pretensioned",
            "Bolt.Type": input_data.get("boltType", ""),
            "Connectivity": input_data.get("connectivity", ""),
            "Connector.Material": input_data.get("material", ""),
            "Design.Design_Method": "Limit State Design",
            "Detailing.Corrosive_Influences": "No",
            "Detailing.Edge_type": "Sheared or hand flame cut",
            "Detailing.Gap": "2",
            "Load.Axial": str(input_data.get("axialForce", "0")),
            "Load.Shear": str(input_data.get("shearForce", "0")),
            "Material": input_data.get("material", ""),
            "Member.Supported_Section.Designation": input_data.get("beamSection", ""),
            "Member.Supported_Section.Material": input_data.get("material", ""),
            "Member.Supporting_Section.Designation": input_data.get("beamSection", ""),
            "Member.Supporting_Section.Material": input_data.get("material", ""),
            "Module": "Beam-Beam End Plate",
            "Weld.Fab": "Shop Weld",
            "Weld.Material_Grade_OverWrite": "410",
            "Connector.Plate.Thickness_List": [str(t) for t in input_data.get("endPlateThickness", [])]
        }
